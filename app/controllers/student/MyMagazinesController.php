...
class MyMagazinesController extends ControllerBase
{
	...
	public function indexAction()
	{
		...
		$query = $this->modelsManager->createQuery('SELECT * FROM NewsApp\Models\Tblmagazine  WHERE profile_id = :profile_id:  ORDER BY timestamp DESC');
		$stmt = $query->execute([
			'profile_id'=>$profile_id,
		]);
		$this->view->magazine = $stmt;
	...
	public function updateAction()
	{
		if(!$this->request->isPost()){
			$this->response->redirect('student/mymagazines');
		}
	...
	public function addcontentAction()
	{
		...
		if($magazineContent->save()==false){
			...
			$this->response->redirect('student/mymagazines/edit/'.$this->request->getPost('magazine_id'));
		}
	}
