	             ...
	             && in_array($file_extension, $validextensions) ) {
					...
					if ($_FILES["image"]["error"] > 0) {
						...
						$orig_name = $_FILES['image']['name'];
			                    ...
			                    // save only if may image (upload image)
			                         // $response = $itemlist->updloadprrofofpayment($_POST,$new_image_name);
								// echo json_encode($response);
			                    $ins = new Employee();

			                    $ins->id	=   $this->request->getPost('id', 'int');
			                    $ins->firstname	=  $this->request->getPost('address');
			                    $ins->lastname	=  $this->request->getPost('lastname');
			                    $ins->age 		=  $this->request->getPost('age','int');
			                    $ins->address 	=  $this->request->getPost('firstname', ['trim', 'string']);
