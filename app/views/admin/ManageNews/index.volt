		...
		<div class="row clearfix">
          ...
          <td>{{ data.timestamp }}</td>
			...
			<ul class="pagination pull-right">
				<li>{{ link_to('admin/managenews', 'First') }}</li>
				<li>{{ link_to('admin/managenews?page=' ~ news.before, 'Previous') }}</li>
				<li>{{ link_to('admin/managenews?page=' ~ news.next, 'Next') }}</li>
				<li>{{ link_to('admin/managenews?page=' ~ news.last, 'Last') }}</li>
			</ul>
		</div>
	</div>
</div>
...
function add_announce()
{
    ...
    function shownews(id)
    {
     save_method = 'edit';
      ...
      $.ajax({
      	...
      	url : "{{ url('admin/managenews/shownews') }}/" + id,
      	type: "GET",
      	dataType: "JSON",
    ...
    function save()
    {
     ...
     if(save_method == 'add')
     {
      url = "{{ url('admin/managenews/createannounce') }}";

    }
    else
    {
      url = "{{ url('admin/managenews/editannounce') }}";
    }
     ...
     function deletenews(id){
      ...
      if(confirm('Are you sure delete this data?'))
      {
       ...
       $.ajax({
        url : "{{ url('admin/managenews/deletenews') }}/" + id,         
        type: "POST",
        dataType: "JSON",
        success: function(data)
        {   
